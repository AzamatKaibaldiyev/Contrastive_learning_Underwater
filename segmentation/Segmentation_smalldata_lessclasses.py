...
wandb.init(
    ...
    "learning_rate": lr,
    "architecture": "UNET",
    "dataset": "Sweden_small_less_classes",
    "epochs": epochs,
    }
)
log_every = 50
test_every = 50
save_checkpoint = "SwedenData/checkpoints/U-Net_normranged_normalized_smalldata_lessclasses_1000epoch_bs32_local.pth"
######################################
...
for epoch in tqdm(range(epochs)):
    ...
    if epoch % log_every==0:
        ...
        report = classification_report(y_preds_flat, y_trues_flat,labels=[0, 1, 2, 3,4], output_dict = True)
...
y_preds_flat = torch.vstack(Y_pred_list).data.cpu().numpy().reshape(1,-1)[0]
y_trues_flat = torch.vstack(Y_list).data.cpu().numpy().reshape(1,-1)[0]
report = classification_report(y_preds_flat, y_trues_flat,labels=[0, 1, 2, 3,4])
print(report)
